...
function ModeratorCreateActivity() {
  ...
  const [expiresDate, setExpiresDate] = useState<Dayjs | null>(now);
  const [expiresTime, setExpiresTime] = useState<Dayjs | null>(now);
  ...
  const handleCreateCodeClick = () => {
    ...
    const expiresError = !expiresDate || !expiresTime ||
      expiresDate.add(expiresTime.hour(), 'hour').add(expiresTime.minute(), 'minute').isAfter(campEndsAt);
      ...
      <div className="flex flex-col gap-6">
        ...
        {step === 2 && (
          <CreateActivityDisplay
            ...
            activityName={activityName}
            coinReward={coinReward}
            expiresAt={expiresDate && expiresTime ? expiresDate.add(expiresTime.hour(), 'hour').add(expiresTime.minute(), 'minute').format('DD/MM/YYYY HH:mm') : ''}
          />
        )}
      </div>
