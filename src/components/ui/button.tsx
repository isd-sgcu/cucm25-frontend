    ...
    "disabled:pointer-events-none disabled:bg-deep-deep-grey [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 ",
  {
    variants: {
      ...
      color: {
        ...
        yellow: 'bg-yellow text-black border-yellow hover:bg-yellow/90 disabled:text-white/70 ',
        white: 'bg-white text-black border-grey hover:bg-grey/10 disabled:text-white/70 ',
        grey: 'bg-grey text-black border-deep-grey hover:bg-deep-grey/10 disabled:text-white/70 ',
        black: 'bg-black text-white border-black hover:bg-black/90 disabled:text-white/70 ',
        red: 'bg-red text-white border-red hover:bg-red/90 disabled:text-white/70 ',
        green: 'bg-green text-white border-green hover:bg-green/90 disabled:text-white/70 ',
      },
      cartoonish: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, color, asChild = false, cartoonish = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ref={ref}
        data-slot='button'
        className={cn(buttonVariants({ variant, size, color, cartoonish, className }))}
        {...props}
      />
    )
  }
